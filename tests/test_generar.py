"""generar.py con un LLM falso: reparto, filtros y CSV."""
import random
import tempfile
import unittest
...
class SynthChecks(unittest.TestCase):
    def test_generates_balanced_rows_without_leaks_and_appends(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows, cost = generar.generate(FakeLLM(), 72, "tickets de un hospital", path, seed=1)
            ...
            generar.generate(FakeLLM(), 36, "otro contexto", path, seed=2)  # Se añade sin repetir la cabecera.
            self.assertEqual(len(generar.read(path)), 108)

    def test_categories_limit_the_combinations_and_prompt_demands_signals(self):
        with tempfile.TemporaryDirectory() as tmp:
            rows, _ = generar.generate(FakeLLM(), 12, "banco", Path(tmp) / "c.csv", seed=3, categories=["seguridad"])
        self.assertEqual({r["categoria"] for r in rows}, {"seguridad"})
        self.assertEqual(len(rows), 12)
        prompt = generar.prompt_for(("seguridad", "alta", False), "banco", [], random.Random(0))
        self.assertIn(generar.SIGNALS["seguridad"], prompt)
        self.assertEqual(set(generar.SIGNALS), set(CATEGORIES))
    ...
    def test_parse_rejects_free_text_and_thin_tickets(self):
        ...
        self.assertEqual(generar.parse(good)[0]["titulo"], "VPN caída")  # Se recortan los espacios.
        for bad in ("**Ticket 1**\nSujeto: VPN", good.replace(words, "muy corta"), good.replace('"Ana"', '""'),
                    good.replace('"area"', '"extra": 1, "area"')):
            with self.assertRaises(generar.ValidationError):
                generar.parse(bad)
    ...
    def test_fallback_switches_only_when_the_first_runs_out_of_credit(self):
        ...
        llm = generar.Fallback(Failing("HTTP 402: Insufficient credits"), second)
        ...
        with self.assertRaises(RuntimeError):  # Otros errores no cambian de proveedor.
            generar.Fallback(Failing("HTTP 500: caído"), second)("x")

    def test_leaks_detects_hints_and_category_names(self):
        self.assertTrue(generar.leaks("Parece un problema de Seguridad", "seguridad"))
        self.assertTrue(generar.leaks("no hay indicios de ataque", "correo"))
        self.assertFalse(generar.leaks("La VPN se desconecta cada hora", "redes"))
