"""Fine-tune de Laya Multilingual para la Mesa de ayuda, con validación y comparación contra la base.
    ...
    .venv/bin/python generar.py --contextos data/contextos.txt --n 216 --backend openrouter   # datos
    .venv/bin/python entrenar.py              # prueba: 72 casos, 1 época, tope de 3 GB de GPU
    .venv/bin/python entrenar.py --completo   # todos los casos, 4 épocas, todo el modelo (GPU de 12 GB o más)

Fases:
1. Lee los casos de data/sintetico.csv, que escribe generar.py con la respuesta decidida de antemano.
...
6. Se compara en validación y en los 20 tickets de prueba (tarea.TICKETS), que nunca entran al entrenamiento.
...
import modelo  # Antes que torch: fija HF_HOME, HF_TOKEN y TORCH_DISABLE_NATIVE_JIT.
...
import evaluar
import generar
from llm import JevModel
from tarea import CATEGORIES, PRIORITIES, QUESTIONS, TICKETS, ticket_state
...
class Trainer:
    def __init__(self, cfg):
        ...
        self.agent = modelo.load(device="cuda")
    ...
    def evaluate(self, dataset):
        return evaluar.evaluate(self.agent, dataset)
    ...
    def save(self, out_dir, fitted, report):
        ...
        base_dir = modelo.base_dir(["tokenizer/*", "encoder/*"])
        ...
        config = {**self.agent.cfg, "fine_tuned": True, "model_name": "laya-mesa-de-ayuda", "temperature": fitted,
                  "fine_tuned_from": modelo.CHECKPOINT}
        ...
        torch.cuda.empty_cache()
        reloaded = modelo.load(out_dir, device="cuda")
...
def compare(base_val, ft_val, base_test, ft_test, base_rows, ft_rows):
    print(evaluar.table({"Validación: base": base_val, "Validación: ajustada": ft_val,
                         "Prueba: base": base_test, "Prueba: ajustada": ft_test}))
    changes = [(b, f) for b, f in zip(base_rows, ft_rows) if (b["category"], b["priority"]) != (f["category"], f["priority"])]
    print(f"\n{len(changes)} tickets de prueba cambian de categoría o prioridad:")
    for b, f in changes:
...
def main():
    ...
    parser.add_argument("--datos", type=Path, default=generar.CSV_PATH, help="CSV que escribe generar.py")
    parser.add_argument("--epocas", type=int, help="épocas (prueba: 1, completo: 4); se guarda la mejor en validación")
    parser.add_argument("--salida", type=Path, help="carpeta del checkpoint (por defecto .model-cache/laya-mesa-de-ayuda)")
    ...
    cfg["epochs"] = args.epocas or cfg["epochs"]
    random.seed(SEED)
    torch.manual_seed(SEED)
    out_dir = args.salida or ROOT / ".model-cache" / f"laya-mesa-de-ayuda{cfg['suffix']}"
    ...
    if cfg["gpu_limit_gb"]:
        ...
        if free < cfg["gpu_limit_gb"] * 2**30:
            raise SystemExit(f"Solo hay {free / 2**30:.1f} GB libres en la GPU: cierra otros procesos que la usen "
                             f"(Laya ocupa 1,6 GB; Ollama con gemma3:12b, unos 9 GB).")
    ...
    print("\n## 1. Casos")
    cases = pick(generar.read(args.datos), cfg["cases_per_spec"])
    if not cases:
        raise SystemExit(f"No hay casos en {args.datos}: genéralos con «python generar.py --n 720».")
    print(f"{len(cases)} casos de {args.datos}")
    print("\n## 2. Profesor")
    teacher = load_teacher(cases, not args.sin_profesor)
    train, calib, val = split(cases, teacher)
    print(f"entrenamiento {len(train)} · calibración {len(calib)} · validación {len(val)} · prueba {len(TICKETS)} (nunca se entrena)")
    ...
    print(f"Validación: categoría {base_val['category_correct']}/{base_val['category_total']} · "
          f"prueba: categoría {base_test['category_correct']}/{base_test['category_total']}")
    ...
    trainer.save(out_dir, fitted, {
        ...
        "validacion": {"base": base_val, "ajustada": ft_val}, "prueba": {"base": base_test, "ajustada": ft_test}})
