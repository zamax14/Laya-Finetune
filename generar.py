"""Datos sintéticos para la Mesa de ayuda: un LLM escribe textos con la respuesta decidida de antemano y se guardan en CSV.
    ...
    python generar.py --prompt "incidencias de TI en un hospital" --n 100            # Ollama local (gemma3:12b)
    python generar.py --prompt "reportes de clientes de un banco" --modelo qwen3-coder:30b
    python generar.py --n 720 --backend openrouter                                    # GPT-5.6 Luna vía OpenRouter
    python generar.py --n 720 --backend openai                                        # gpt-6-luna con la API de OpenAI
...
repetidos, en el CSV o respecto de los 20 tickets de prueba. Las filas se añaden a data/sintetico.csv, que es lo
que lee entrenar.py para reentrenar.
"""
import argparse
import csv
import hashlib
...
from llm import OPENROUTER, post
from tarea import CATEGORIES, LEAK_HINTS, PRIORITIES, TICKETS
...
def read(path=CSV_PATH):
    """Filas del CSV con la forma de un ticket de tarea.py, para reutilizar ticket_state y evaluar.row."""
    if not path.exists():
        return []
